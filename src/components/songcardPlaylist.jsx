...
export const SongCardPlaylist = ({ songs, volumen, useEff, analytics, currSong, setCurrSong }) => {
    ...
    useEffect(() => {
        logEvent(analytics, 'playSong', { name: songs[0].song_name, value: songs[0].song_name });
    }, []);
    ...
    const changeSong = (next, skip) => {
        if (skip) {
            logEvent(analytics, 'skipSong', { name: songs[currSong].song_name, value: songs[currSong].song_name });
        }
        if (next >= songs.length) {
            ...
            logEvent(analytics, 'playSong', { name: songs[songs.length - 1].song_name, value: songs[songs.length - 1].song_name });
        } else {
            setCurrSong(next);
            logEvent(analytics, 'playSong', { name: songs[next].song_name, value: songs[next].song_name });
        }
    }
